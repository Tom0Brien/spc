...
void G1Navigation::GetObservation(const mjModel* model, const mjData* data, float* obs_out) const {
    ...
    if (pelvis_linvel_adr_ >= 0) {
        ...
        for (int i = 0; i < 3; ++i) obs_out[idx++] = 0.0f;
    }
...
void G1Navigation::ApplyControl(const mjModel* model, mjData* data, const float* action) const {
    ...
    if (policy_) {
        ...
        for (int i = 0; i < 29; ++i) policy_action[i] = 0.0f;
    }
    ...
    for (int i = 0; i < 29; ++i) {
        ...
        if (motor_target < jnt_range_low_[i]) motor_target = jnt_range_low_[i];
        if (motor_target > jnt_range_high_[i]) motor_target = jnt_range_high_[i];
