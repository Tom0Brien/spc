...
FrankaPush::FrankaPush(mjModel* model, const spc::core::TaskConfig& config) {
    ...
    std::string gripper_name =
        config.string_params.count("gripper_name") ? config.string_params.at("gripper_name") : "gripper";

    obj_body_ = mj_name2id(model, mjOBJ_BODY, obj_name.c_str());
    gripper_site_ = mj_name2id(model, mjOBJ_SITE, gripper_name.c_str());
    mocap_target_ =
        config.numeric_params.count("mocap_target") ? static_cast<int>(config.numeric_params.at("mocap_target")) : 0;
    action_scale_ = config.numeric_params.count("action_scale")
                        ? static_cast<float>(config.numeric_params.at("action_scale"))
                        : 0.1f;

    obj_target_weight_ =
        config.numeric_params.count("obj_target_weight") ? config.numeric_params.at("obj_target_weight") : 10.0;
    gripper_obj_weight_ =
        config.numeric_params.count("gripper_obj_weight") ? config.numeric_params.at("gripper_obj_weight") : 5.0;
    orientation_weight_ =
        config.numeric_params.count("orientation_weight") ? config.numeric_params.at("orientation_weight") : 1.0;
    residual_weight_ =
        config.numeric_params.count("residual_weight") ? config.numeric_params.at("residual_weight") : 0.1;
}
...
void FrankaPush::GetObservation(const mjModel* model, const mjData* data, float* obs_out) const {
    ...
    // 1. target_pos (3)
    for (int i = 0; i < 3; ++i)
        obs_out[idx++] = data->mocap_pos[3 * mocap_target_ + i];
    ...
    for (int i = 3; i < 9; ++i)
        obs_out[idx++] = target_mat[i];

    // 3. last_action (7)
    for (int i = 0; i < 7; ++i)
        obs_out[idx++] = data->ctrl[i] / action_scale_;

    // 4. robot_qpos (7)
    for (int i = 0; i < 7; ++i)
        obs_out[idx++] = data->qpos[i];

    // 5. robot_qvel (7)
    for (int i = 0; i < 7; ++i)
        obs_out[idx++] = data->qvel[i];

    // 6. gripper_pos (3)
    for (int i = 0; i < 3; ++i)
        obs_out[idx++] = data->site_xpos[3 * gripper_site_ + i];

    // 7. gripper_mat.ravel()[3:] (6)
    for (int i = 3; i < 9; ++i)
        obs_out[idx++] = data->site_xmat[9 * gripper_site_ + i];

    // 8. obj_orientation (6)
    mjtNum obj_mat[9];
    mju_quat2Mat(obj_mat, data->xquat + 4 * obj_body_);
    for (int i = 3; i < 9; ++i)
        obs_out[idx++] = obj_mat[i];

    // 9. obj_pos (3)
    for (int i = 0; i < 3; ++i)
        obs_out[idx++] = data->xpos[3 * obj_body_ + i];
...
double FrankaPush::RunningCost(const mjModel* model, const mjData* data, const float* control) const {
    ...
    double norm_img =
        std::sqrt(quat_diff[1] * quat_diff[1] + quat_diff[2] * quat_diff[2] + quat_diff[3] * quat_diff[3]);
    if (norm_img > 1.0)
        norm_img = 1.0;
    ...
    return obj_target_weight_ * obj_target_cost + gripper_obj_weight_ * gripper_obj_cost +
           orientation_weight_ * orientation_cost + residual_cost;
}
